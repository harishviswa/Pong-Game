public class BumperCollision
{
   ...
   private static void findImpactPoint(Bumper bumper, Ball ball)
   {
      ...
      for (int x = bumper.getX() - bumper.getWidth()/2; x <= bumper.getX() + bumper.getWidth()/2; x++)  // top & bottom edges
      {
         updateIfCloser(x, bumper.getY() - bumper.getHeight()/2, ball);
         updateIfCloser(x, bumper.getY() + bumper.getHeight()/2, ball);
      }
      for (int y = bumper.getY() - bumper.getWidth()/2; y <= bumper.getY() + bumper.getHeight()/2; y++)  // right & left edges
      {
